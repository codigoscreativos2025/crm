...
export default function Sidebar() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                // Fetch User Profile for Metrics Access
    ...
    const handleLogout = async () => {
        await signOut({ callbackUrl: 'https://cm.pivotsoluciones.com' });
    };
        ...
        <aside className="flex h-full w-full flex-col border-r border-gray-200 bg-white md:w-96">
            ...
            <div className="flex items-center justify-between bg-[#f0f2f5] p-3 px-4">
                <div className="flex items-center gap-2">
                    ...
                    <Link href="/dashboard/templates" className="p-2 text-blue-500 hover:bg-blue-100 rounded-full transition" title="Respuestas Rápidas">
            ...
            <div className="flex-1 overflow-y-auto bg-white dark:bg-gray-900 transition-colors duration-200">
                {filteredContacts.map((contact) => (
                    <Link
                        ...
                        className="group flex cursor-pointer items-center border-b border-gray-100 dark:border-gray-800 p-3 hover:bg-[#f5f6f6] dark:hover:bg-gray-800 transition-colors duration-200 relative"
                    >
                        ...
                        <div className="flex-1 overflow-hidden border-b border-gray-100 pb-3 pr-3 group-hover:border-none">
                            <div className="flex justify-between items-center mb-1">
                                <span className={`truncate text-base ${contact.unreadCount ? 'font-semibold text-gray-900 dark:text-white' : 'font-normal text-gray-900 dark:text-gray-100'}`}>
                                    {contact.name || contact.phone}
                                </span>
                                <div className="flex flex-col items-end gap-1">
                                    <span className={`text-[11px] ${contact.unreadCount ? 'text-green-500 font-medium' : 'text-gray-400 dark:text-gray-500'}`}>
                                        {contact.messages[0] ? new Date(contact.messages[0].timestamp).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) : ''}
                                    </span>
                                    {contact.unreadCount && contact.unreadCount > 0 ? (
                                        <div className="flex h-5 min-w-[20px] items-center justify-center rounded-full bg-[#25D366] px-1.5 text-[11px] font-bold text-white shadow-sm">
                                            {contact.unreadCount}
                                        </div>
                                    ) : (
                            ...
                            <div className="flex justify-between items-center">
                                <p className={`truncate text-sm max-w-[140px] ${contact.unreadCount ? 'text-gray-700 dark:text-gray-300 font-medium' : 'text-gray-500 dark:text-gray-400'}`}>
                                    {contact.messages[0]?.body || 'Inicia una conversación'}
                                </p>
                                {contact.stage && (
                                    <span className="text-[10px] px-2 py-0.5 rounded-full bg-green-100 text-green-800 border border-green-200 ml-2 whitespace-nowrap">
                                        {contact.stage.name}
                                    </span>
                                )}
                            </div>
