...
export default function FunnelManager() {
    ...
    const handleDeleteFunnel = async (funnelId: number) => {
        if (!confirm('¿Estás seguro de eliminar todo el embudo? Esta acción eliminará también todas sus etapas (pero no los contactos, que quedarán sin etapa).')) return;
        try {
            const res = await fetch(`/api/funnels/${funnelId}`, { method: 'DELETE' });
            if (res.ok) {
                fetchFunnels();
            } else {
                alert('Hubo un error al eliminar el embudo.');
            }
        } catch (error) {
            console.error(error);
        }
            ...
            <div className="grid gap-6">
                {funnels.map(funnel => (
                    <div key={funnel.id} className="border border-gray-200 rounded-lg overflow-hidden">
                        <div className="bg-gray-100 p-4 font-semibold text-gray-700 border-b border-gray-200 flex justify-between items-center group">
                            <div className="flex items-center gap-2">
                                <h3>{funnel.name}</h3>
                                <span className="text-xs text-gray-400 font-normal">ID: {funnel.id}</span>
                            </div>
                            <button
                                onClick={() => handleDeleteFunnel(funnel.id)}
                                className="text-gray-400 hover:text-red-500 hover:bg-white p-1.5 rounded transition opacity-0 group-hover:opacity-100 shadow-sm"
