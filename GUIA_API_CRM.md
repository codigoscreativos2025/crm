...
### D. Marcar un Chat como Leído (`PUT /api/contacts/[id]/read`)

Si estás utilizando el chat desde una interfaz externa o necesitas limpiar el indicador de mensajes no leídos del CRM programáticamente, puedes usar este endpoint.

#### Caso 1: Marcar Chat Leído

Se requiere el `ID` numérico del contacto (obtenido previamente vía GET o webhook).
Usa la cabecera estándar de `cookie/session` si llamas desde el frontend del CRM, o pásalo de forma controlada si tuvieras autenticación extra adaptada para este entorno.

```bash
curl -X PUT https://tudominio.com/api/contacts/123/read \
-H "Content-Type: application/json"
```

**Resultado:**
Todos los mensajes (`isReadByAgent: false`) asociados a ese contacto cambiarán a `true`, reseteando el contador de notificaciones del sidebar del asesor comercial.
