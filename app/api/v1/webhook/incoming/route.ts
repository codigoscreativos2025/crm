import { NextRequest, NextResponse } from "next/server";
import prisma from "@/lib/prisma";
import { authenticateApiKey } from "@/lib/auth";
import { startCleanupScheduler } from "@/lib/image-cleanup";

startCleanupScheduler();

export async function POST(req: NextRequest) {
    try {
        const body = await req.json();
        const { userApiKey, contactPhone, contactName, message, timestamp, stageId, mediaUrl, mediaType, mediaName } = body;

        // 1. Authenticate Request
        const user = await authenticateApiKey(userApiKey);
        if (!user) {
            return NextResponse.json({ error: "Unauthorized" }, { status: 401 });
        }

        if (!contactPhone || !message) {
            return NextResponse.json({ error: "Missing required fields" }, { status: 400 });
        }

        // 2. Default Stage Logic
        let defaultStageId = stageId ? parseInt(stageId) : undefined;
        if (!defaultStageId && user.defaultFunnelId) {
            // Find the first stage of the default funnel
            const defaultStage = await prisma.stage.findFirst({
                where: { funnelId: user.defaultFunnelId },
                orderBy: { order: 'asc' }
            });
            if (defaultStage) {
                defaultStageId = defaultStage.id;
            }
        }

        // 3. Find or Create Contact
        const contact = await prisma.contact.upsert({
            where: {
                userId_phone: {
                    userId: user.id,
                    phone: contactPhone,
                },
            },
            update: {
                // Update name if provided and previously null, or just keep latest
                name: contactName || undefined,
                stageId: stageId ? parseInt(stageId) : undefined, // Only update stage if explicitly passed
            },
            create: {
                phone: contactPhone,
                name: contactName,
                userId: user.id,
                stageId: defaultStageId,
            },
        });

        // 3. Save Message
        const newMessage = await prisma.message.create({
            data: {
                body: message,
                direction: "inbound",
                status: "received",
                timestamp: timestamp ? (typeof timestamp === 'number' && timestamp < 10000000000 ? new Date(timestamp * 1000) : new Date(timestamp)) : new Date(),
                contactId: contact.id,
                fileUrl: mediaUrl || null,
                fileType: mediaType || null,
                fileName: mediaName || null,
            },
        });

        return NextResponse.json({ success: true, messageId: newMessage.id });
    } catch (error) {
        console.error("Error in incoming webhook:", error);
        return NextResponse.json({ error: "Internal Server Error" }, { status: 500 });
    }
}
